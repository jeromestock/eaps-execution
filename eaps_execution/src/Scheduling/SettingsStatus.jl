...
"""
Create a spaces.Discrete space for gymnasium environments.
...
"""
Create a spaces.MultiDiscrete space for gymnasium environments.
...
"""
Create a spaces.Dict space for gymnasium environments.
...
"""
Set the observation_space and action_space of the gymnasium environment.
...
:param random: Set random number (i.e. envset.pyenv."np_random") to slice scenario data.
:param keep_date: Keep date column in dataframe.
"""
function py_import_scenario(envset::EnvironmentSettings, scenario_paths; prefix_renamed = false, random = false, keep_date = false)
    paths = String[]
    ...
    for path in scenario_paths
        ...
        if haskey(path, "date_format")
            push!(time_conversion_str, get(path, "time_conversion_str", path["date_format"]))
        else
    ...
    df = declaremissings(DataFrame(scenario.values, scenario.columns.to_list()); values=(NaN, "NULL", nothing))

    df = keep_date == true ? hcat(df, DataFrame(date=DateTime.(scenario.index))) : df
