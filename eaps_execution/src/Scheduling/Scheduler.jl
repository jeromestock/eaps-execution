...
function mapevents(prodsystem::ProductionSystem; debug=false)
    ...
    for (j_idx, job) in pairs(prodsystem.jobs), (o_idx, op) in pairs(prodsystem.products[job.product].operations)
        ...
        if o_idx > 1
            priorsteps = collect(length(events)-2*job.quantity+1:length(events)-job.quantity)
            currentsteps = collect(length(events)-job.quantity+1:length(events))
    ...
    for i in 1:count
        if lowercase(method) == "spt"
            try
                ...
                show(err)
                error("Error when generating SPT schedule.")
            end
        else
